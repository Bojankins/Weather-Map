(function() {
  ...
  function getJSON() {
    ...
    var url = 'http://api.wunderground.com/api/8602561c1a1ca88c/forecast10day/q/' + zip + '.json?callback=?';
    $.getJSON(url, weather);
  }
  function weather(cond) {
    var forecast = cond.forecast.simpleforecast.forecastday;
    var zip = $('#zip').val().trim();
    $('#graphs').append(("<div class='graph' data-zip=" + zip + "></div>"));
    makeNewChart(zip);
    forecast.forEach((function(w) {
